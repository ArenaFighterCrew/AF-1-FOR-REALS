...
public class PlayState extends State {
    ...
    */
    ...
    private void death(){
        ...
        if(!map.getBounds1().contains(player1.getBounds()) && !player1.getDead()){
            player1.setDead(true);
            ArenaFighter.death.play(0.2f);
        }

        if(!map.getBounds1().contains(player2.getBounds()) && !player2.getDead()){
            player2.setDead(true);
            ArenaFighter.death.play(0.2f);
        }

        if(!map.getBounds1().contains(player3.getBounds()) && !player3.getDead()){
            player3.setDead(true);
            ArenaFighter.death.play(0.2f);
        }

        if(!map.getBounds1().contains(player4.getBounds()) && !player4.getDead()){
        ...
        else if (player1.getDead() && player2.getDead() && player3.getDead() && player4.getDead()){
            canScore = false;
            delay();
        }
